...
const getPost = (params) => getData(`${refs.apiUrl}${params}`)
    .then(data => renderPost(data))
    .catch(err => console.error(err));

const getComments = (params) => getData(`${refs.apiUrl}${params}`)
    .then(data => data.forEach(obj => renderComments(obj)))
    .catch(err => console.error(err));
...
const handlerSubmitForm = async (e) => {
    ...
    await getPost(`/posts/${idPost}`);

    refs.postForm.reset();
};

const handlerClickBtn = async (e) => {
    e.preventDefault();

    const idComments = e.target.dataset.id;
    await getComments(`/comments?postId=${idComments}`);
};
